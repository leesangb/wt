...
표시합니다. Space를 focus하면 즉시 갱신하고, focus 중에는 30초마다,
background에서는 5분마다 갱신합니다.
...
`wt init`는 wt의 머신별 설정과 task artifact를 저장소 로컬 `.git/info/exclude`에 추가합니다. 따라서 커밋되는 `.gitignore` 파일을 수정하지 않고도 이 파일들이 기본적으로 추적되지 않습니다.
